...
<head>
	...
	<title>Cetak Data Pembinaan</title>
</head>
	...
	<center>
		...
		<center>
			<table class="kop">
				...
				<td style="font-size: 30px; font-weight: bold;">Data Pembinaan</td>
			</table>
		</center>
		<hr size="2px" color="black" style="margin-top: 1px;">
		<dv>
			...
			<tbody>
				<tbody>
					<?php 
					$no = 1;
					$sql = mysqli_query($koneksi,"SELECT * FROM pembinaan_organisasi 
						INNER JOIN data_organisasi ON data_organisasi.id_organisasi=pembinaan_organisasi.id_organisasi");
					while ($data = mysqli_fetch_array($sql)) {
						...
						<tr class="gradeA">
							...
							<td><?=$data['tanggal_pembinaan'];?></td>
							<td><?=$data['no_telpon'];?></td>
							<td><?=$data['alamat'];?></td>
							<td><?=$data['keterangan_pembinaan'];?></td>
